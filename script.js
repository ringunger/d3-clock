...
function drawCanvas(){
    ...
    for(var i = 0; i < 60; i++){
        ...
        if(i % 5 == 0) {
            ...
            .attr("x", center_x - (font_size / 2))
            .attr("y", radius +  font_size + tick_l + 2)
            .attr("transform", "rotate(" + i * 6 + "," + center_x + "," + center_y + ")")
    ...
    svg.append("text").attr("class", "clock-logo").attr("x", center_x - 26).attr("y", center_y + (radius / 2))
                        .style("fill", "#ddd")
                        .text("RingleSoft");

    svg.append("line").attr("class", "arrow second")
    .attr("x1", center_x).attr("y1", center_y + 12)
    .attr("x2", center_x).attr("y2", (radius + (0.2 * radius)));
    ...
    .attr("x1", center_x).attr("y1", center_y + 12)
    .attr("x2", center_x).attr("y2", (radius + (0.45 * radius)));
